...
#include <boost/hydra/core/span.hpp>
#include <boost/hydra/rtsp/message.hpp>
#include <boost/hydra/rtsp/serializer.hpp>
#include <memory>
...
/** Type-erased buffers generator for @ref rtsp::message
   
    Implements the BuffersGenerator concept for any concrete instance of the
    @ref rtsp::message template.
   
    @ref rtsp::message_generator takes ownership of a message on construction,
    ...
    rtsp::message_generator handle_request(
        string_view doc_root,
        rtsp::request<Body, Fields>&& request);
    @endcode
   
    The @ref hydra::write and @ref hydra::async_write operations are provided
    for BuffersGenerator. The @ref rtsp::message::keep_alive property is made
...
public:
    ...
    message_generator(rtsp::message<isRequest, Body, Fields>&&);
...
#include <boost/hydra/rtsp/impl/message_generator.hpp>
