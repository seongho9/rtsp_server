    ...
    char const* last, error_code& ec)
{
    ...
    hydra::detail::char_buffer<max_obs_fold> buf;
    auto p = in;
    for(;;)
    {
        if(p + 2 > last)
...
finish_header(error_code& ec, std::false_type)
{
        ...
        status_ == 304)          // Not Modified
    {
        ...
        //        https://github.com/boostorg/hydra/issues/692
        state_ = state::complete;
    }
    else if(f_ & flagContentLength)
    {
    ...
    string_view value, error_code& ec)
{
    ...
    if(f == field::connection)
    {
        ...
        {
            if(hydra::iequals("close"_sv, s))
            {
                f_ |= flagConnectionClose;
                continue;
            }

            if(hydra::iequals("keep-alive"_sv, s))
            {
                f_ |= flagConnectionKeepAlive;
                continue;
            }

            if(hydra::iequals("upgrade"_sv, s))
            {
    ...
    //         [&](string_view const& s)
    //         {
    //             return hydra::iequals("chunked"_sv, s);
    //         });
    //     if(p == v.end())
    //         return;
    //     if(std::next(p) != v.end())
...
template class rtsp::basic_parser<true>;
template class rtsp::basic_parser<false>;
#endif
