...
#include <boost/hydra/rtsp/type_traits.hpp>
#include <boost/hydra/rtsp/error.hpp>
#include <boost/hydra/rtsp/parser.hpp>
#include <boost/hydra/rtsp/read.hpp>
#include <boost/hydra/core/async_base.hpp>
#include <boost/hydra/core/stream_traits.hpp>
#include <boost/hydra/core/detail/buffer.hpp>
#include <boost/hydra/core/detail/read.hpp>
...
template<
    ...
    class Handler>
class read_msg_op
    : public hydra::stable_async_base<
        Handler, hydra::executor_type<Stream>>
...
public:
            ...
            *this, s, m))
    {
        ...
        rtsp::async_read(d_.s, b, d_.p, std::move(*this));
    }
...
struct run_read_msg_op
{
            ...
            basic_fields<Allocator>>* m)
    {
        ...
        static_assert(
            hydra::detail::is_invocable<ReadHandler,
        ...
        BOOST_ASIO_CORO_REENTER(*this)
        {
            ...
            do_read:
                BOOST_ASIO_CORO_YIELD
                {
                    ...
                    auto const mb =
                        hydra::detail::dynamic_buffer_prepare(
        ...
        upcall:
            if(! cont_)
            {
                BOOST_ASIO_CORO_YIELD
                {
                    ...
                    net::dispatch(
                        ex,
                        hydra::bind_front_handler(std::move(self), ec));
                }
            }
    ...
    basic_parser<isRequest>& parser)
{
    ...
    auto const bytes_transferred =
        rtsp::read_some(stream, buffer, parser, ec);
    if(ec)
    ...
    ReadHandler&& handler)
{
    return net::async_compose<ReadHandler,
        void(hydra::error_code, std::size_t)>(
    ...
    basic_parser<isRequest>& parser)
{
    ...
    error_code ec;
    auto const bytes_transferred =
        rtsp::read_header(stream, buffer, parser, ec);
    if(ec)
        BOOST_THROW_EXCEPTION(system_error{ec});
    return bytes_transferred;
}
    ...
    basic_parser<isRequest>& parser)
{
    ...
    error_code ec;
    auto const bytes_transferred =
        rtsp::read(stream, buffer, parser, ec);
    if(ec)
        BOOST_THROW_EXCEPTION(system_error{ec});
    return bytes_transferred;
}
    ...
    message<isRequest, Body, basic_fields<Allocator>>& msg)
{
    ...
    error_code ec;
    auto const bytes_transferred =
        rtsp::read(stream, buffer, msg, ec);
    if(ec)
        BOOST_THROW_EXCEPTION(system_error{ec});
    return bytes_transferred;
}
