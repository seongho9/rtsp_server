...
public:
    ...
    ~handler()
    {
        ::boost::hydra::unit_test::suite::this_suite()->expect(pass_, loc_.file_name(), loc_.line());
    }
    ...
    operator()(error_code ec, Args&&...)
    {
        ::boost::hydra::unit_test::suite::this_suite()->expect(!pass_, loc_.file_name(), loc_.line());
        if (ec_ && ec != *ec_)
            ::boost::hydra::unit_test::suite::this_suite()->fail(ec.message(), loc_.file_name(), loc_.line());
        else
            ::boost::hydra::unit_test::suite::this_suite()->pass();
        pass_ = true;
    }
    ...
    operator()()
    {
        ::boost::hydra::unit_test::suite::this_suite()->expect(!pass_, loc_.file_name(), loc_.line());
        if (ec_ && ec_->failed())
            ::boost::hydra::unit_test::suite::this_suite()->fail(ec_->message(), loc_.file_name(), loc_.line());
        else
            ::boost::hydra::unit_test::suite::this_suite()->pass();
