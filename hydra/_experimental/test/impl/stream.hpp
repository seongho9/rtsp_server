...
#include <boost/hydra/core/detail/service_base.hpp>
...
#if defined(BOOST_ASIO_NO_TS_EXECUTORS)
            net::dispatch(wg2_,
                hydra::bind_front_handler(std::move(h_),
...
public:
    ...
    operator()(error_code ec) override
    {
        ...
        net::post(wg1_.get_executor(),
            hydra::bind_front_handler(std::move(fn_), ec));
        wg1_.reset();
#else
        net::post(wg1_, hydra::bind_front_handler(std::move(fn_), ec));
...
struct basic_stream<Executor>::run_write_op
{
        ...
        ConstBufferSequence const& buffers)
    {
        ...
        auto const upcall = [&](error_code ec, std::size_t n)
        {
            net::post(
                in_->exec,
                hydra::bind_front_handler(std::move(h), ec, n));
        };
