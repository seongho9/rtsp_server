...
#include <boost/hydra/core/detail/stream_base.hpp>
#include <boost/hydra/core/detail/is_invocable.hpp>
#include <boost/hydra/core/error.hpp>
#include <boost/hydra/core/rate_policy.hpp>
#include <boost/hydra/core/role.hpp>
...
/** A stream socket wrapper with timeouts, an executor, and a rate limit policy.
    ...
    operations will be invoked with the error @ref hydra::error::timeout.
    ...
    {
        flat_buffer buffer;
        rtsp::request<rtsp::empty_body> req;

        // Read the request, with a 15 second timeout
        stream.expires_after(std::chrono::seconds(15));
        rtsp::async_read(stream, buffer, req, yield);

        // Calculate the response
        rtsp::response<rtsp::string_body> res = make_response(req);

        // Send the response, with a 30 second timeout.
        stream.expires_after (std::chrono::seconds(30));
        rtsp::async_write (stream, res, yield);
    }
    @endcode
...
public:
    ...
    */
    using executor_type = hydra::executor_type<socket_type>;
...
private:
        ...
        , boost::empty_value<RatePolicy>
    {
        ...
        hydra::executor_type<socket_type>
        ex() noexcept
        {
...
public:
    ...
    */
    ...
    /** Set the timeout for the next logical operation.
        ...
        error @ref hydra::error::timeout. Otherwise, if the timer
    ...
    */
    ...
    /** Set the timeout for the next logical operation.
        ...
        operations will complete with the error @ref hydra::error::timeout.
...
#include <boost/hydra/core/impl/basic_stream.hpp>
