...
#include <boost/hydra/core/flat_static_buffer.hpp>
#include <boost/hydra/core/read_size.hpp>
...
public:
        ...
        bool cont = true)
    {
        ...
        BOOST_ASIO_CORO_REENTER(*this)
        {
            for(;;)
            {
                max_prepare = hydra::read_size(b_, cond_(ec, total_, b_));
                if(max_prepare == 0)
                    break;
    ...
    error_code& ec)
{
    ...
    for(;;)
    {
        max_prepare =  hydra::read_size(buffer, cond(ec, total, buffer));
        if(max_prepare == 0)
            break;
