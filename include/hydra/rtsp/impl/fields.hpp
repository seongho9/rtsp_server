...
#include <boost/hydra/core/buffers_cat.hpp>
#include <boost/hydra/core/string.hpp>
#include <boost/hydra/core/detail/buffers_ref.hpp>
#include <boost/hydra/core/detail/clamp.hpp>
#include <boost/hydra/core/detail/static_string.hpp>
#include <boost/hydra/core/detail/temporary_buffer.hpp>
#include <boost/hydra/core/static_string.hpp>
#include <boost/hydra/rtsp/verb.hpp>
#include <boost/hydra/rtsp/rfc7230.hpp>
#include <boost/hydra/rtsp/status.hpp>
#include <boost/hydra/rtsp/chunk_encode.hpp>
...
public:
    using const_buffers_type =
        hydra::detail::buffers_ref<view_type>;
    ...
    string_view sname, string_view const& value)
{
    ...
    if(! hydra::iequals(sname, last->name_string()))
    {
        BOOST_ASSERT(count(sname) == 0);
        set_.insert_before(before, e);
        list_.push_back(e);
...
struct iequals_predicate
{
    ...
    {
        return hydra::iequals(s, sv1) || hydra::iequals(s, sv2);
    }
...
filter_token_list_last(
    hydra::detail::temporary_buffer& s,
    string_view value,
...
void
keep_alive_impl(
    hydra::detail::temporary_buffer& s, string_view value,
...
{
    // transfer-encoding is use in HTTP
    ...
    //     if(next == te.end())
    //         return hydra::iequals(*it, "chunked");
    //     it = next;
    // }
    return false;
}
...
{
    //  transfer-encoding is used in HTTP
    // hydra::detail::temporary_buffer buf;
...
set_element(element& e)
{
    ...
    for(;;)
    {
        ...
        it = next;
        if(it == set_.end() ||
            ! hydra::iequals(e.name_string(), it->name_string()))
            break;
    }
    set_.insert_before(it, e);
    list_.push_back(e);
...
realloc_string(string_view& dest, string_view s)
{
    if(dest.empty() && s.empty())
        return;
    auto a = typename hydra::detail::allocator_traits<
